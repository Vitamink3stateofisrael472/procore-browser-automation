/**
 * anchor-cloud.js — AnchorBrowser cloud example for Procore
 *
...
const { login_procore, create_rfi } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Procore automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_procore(page, {});
      log('Action 1 result:', data1);

      const data2 = await create_rfi(page, {});
